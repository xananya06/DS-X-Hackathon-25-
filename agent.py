...
class ConsciousCartAgent:
    ...
    def __init__(self):
        ...
        self.conversation_history = []
        self.tool_calls = []
        self.all_session_tool_calls = []  # Track all tools across queries
        ...
        self.tools = [
            ...
            },
            {
                ...
                "description": "Get Amazon product link for a specific product. Use this when recommending alternatives to provide shopping links.",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "product_name": {
                            "type": "string",
                            "description": "Full product name including brand (e.g., 'e.l.f. Big Mood Mascara')"
                        },
                        "product_category": {
                            "type": "string",
                            "description": "Product category (e.g., 'mascara', 'foundation', 'lipstick')"
                        }
                    },
    ...
    def get_total_brands_count(self) -> int:
    ...
    def get_session_brands_count(self) -> int:
        """Get number of unique brands checked in this session"""
        unique_brands = set()
        for tool_call in self.all_session_tool_calls:
            if tool_call["tool"] == "check_database":
                brand_name = tool_call["input"].get("brand_name", "")
                if brand_name:
                    unique_brands.add(brand_name.lower())
        return len(unique_brands)
    
    def get_cruelty_free_count(self) -> int:
        """Get number of cruelty-free brands in database"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
    ...
    def _get_amazon_link(self, product_name: str, product_category: str) -> dict:
        """Tool: Generate Amazon search link for a product"""
        ...
        print(f"[Amazon Link] Generated for: {product_name}")
...
For product recommendations, format as:
PRODUCT: [Brand Name Product Name] ($price) - [description]
...
PETA Cruelty-Free Database: 
PRODUCT: e.l.f. Big Mood Mascara ($7) - Certified cruelty-free, 100% vegan
PRODUCT: Essence Lash Princess Mascara ($5) - Cruelty-free verified

Leaping Bunny Certified:
PRODUCT: Pacifica Dream Big Mascara ($12) - Certified CF & vegan
PRODUCT: Milk Makeup Kush Mascara ($24) - Leaping Bunny approved
...
PETA Database:
PRODUCT: e.l.f. Flawless Finish Foundation ($7) - 100% vegan, CF certified
PRODUCT: Pacifica Alight Multi-Mineral Foundation ($14) - Vegan, CF

Leaping Bunny:
PRODUCT: Physicians Formula Healthy Foundation ($13) - Certified cruelty-free
PRODUCT: Cover FX Power Play Foundation ($48) - Luxury CF option
...
PETA Database:
PRODUCT: e.l.f. O Face Satin Lipstick ($4) - Certified cruelty-free, vegan
PRODUCT: NYX Professional Makeup Matte Lipstick ($6) - CF verified

Leaping Bunny:
PRODUCT: Pacifica Devocean Natural Lipstick ($10) - Certified CF & vegan
PRODUCT: Bite Beauty Power Move Lipstick ($24) - Leaping Bunny approved
...
CONFIDENCE: Medium (using cached knowledge)"""
    ...
    def _execute_tool(self, tool_name: str, tool_input: dict) -> any:
        ...
        self.all_session_tool_calls.append(tool_record)
        ...
        elif tool_name == "get_amazon_link":
            return self._get_amazon_link(
...
2. If not found or stale, use web_search to verify
3. When recommending alternatives, ALWAYS respect user constraints: {constraints}
4. For EACH alternative product you recommend, use get_amazon_link tool to provide a shopping link
5. Save new verifications to database
6. Be conversational and remember the user's preferences

IMPORTANT:
- When suggesting alternatives, filter by user's budget if known
- Prioritize options that match ALL user values (vegan, fragrance-free, etc.)
- ALWAYS include Amazon links for recommended products
- Format links as markdown: [Product Name](Amazon Link)
- Mention when products match user preferences
- Be friendly and personal
- If user asks a follow-up question like "is it vegan?", refer to the last brand discussed: {self.last_brand_discussed or 'unknown'}
...
RESPONSE STYLE:
- Conversational and warm
- Include clickable Amazon links for all product recommendations
- Acknowledge user preferences when relevant
- Explain WHY recommendations match their needs"""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("Testing Agent with Amazon Links")
    print("="*60)
    
    response, tools = agent.process_query("What are cruelty-free mascara alternatives?")
    print(f"\nResponse: {response}")
    print(f"\nTools used: {len(tools)}")
    
    # Check if Amazon links were fetched
    amazon_tools = [t for t in tools if t["tool"] == "get_amazon_link"]
    print(f"\nAmazon links generated: {len(amazon_tools)}")
    for tool in amazon_tools:
        print(f"- {tool['input']['product_name']}")
